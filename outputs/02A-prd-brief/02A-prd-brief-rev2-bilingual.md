# PRD Brief rev2 — Mistfall Hunter 决策引擎工具+攻略站
# PRD Brief rev2 — Mistfall Hunter Decision-Engine Tool & Guide Site

> **阶段 Stage**: 02A-rev PRD Brief rev2（产品边界修订确认件 / revised product boundary confirmation）
> **日期 Date**: 2026-08-08
> **作者 Author**: prd (moce)，基于 02A v1 + owner 部分修改确认（t_28675e89）
> **状态 Status**: [NEEDS_REVIEW] — 等待 owner 对 rev2 修订项（确认清单 #2 / #6）做最终确认（02B）
> **语言 Language**: 中英双语。英文保留关键词/路由/标题供 SEO/copy/frontend 下游直接使用；中文解释业务含义与确认点。
> **Supersedes / 取代**: outputs/02A-prd-brief/02A-prd-brief-bilingual.md（v1）。v1 保留为历史版本，不再作为确认依据。

## 修订说明 / Revision Notes（rev2 vs v1）

**未改动章节 / Unchanged chapters**：§1 市场概述、§3 目标用户（ICP×4 与用户任务链）、§5.0-5.5 工具与内容规划、§6 页面信息架构、§7 Route Contract（58 路由）、§8 域名与技术栈、§10 转化漏斗与埋点、§12 交接摘要结构。

**修订章节与依据 / Revised chapters**（owner 决策记录：inputs/02-owner-confirmations/owner-decision-02A-rev-20260808.md）：

| 修订项 | owner 指令 | 落点 |
|---|---|---|
| M1 | #2 定位：肯定要有游戏数据；允许汲取/复用竞品优点，包括竞品与官方的图片、数据、文案方向和主题，必要时可直接用 | §0 / §2.2 / §4 定位 |
| M2 | #6 NOT-DO：确认要扒游戏数据；允许复用竞品/官方素材；logo 不用官方本体，做类似官方风格的原创 logo | §4 NOT-DO（12 条重写为可执行口径） |
| M3 | IP/版权/游戏 ToS/商标相似风险写入风险登记；owner 明确决策接受方向，交 04 合规评估缓解措施与 takedown 预案 | §11 风险登记（R8-R11 新增） |

**连带一致性修改 / Consistency edits**（口径与 M1/M2 直接冲突的最小必要同步）：§5.6 数据合同基线、§9 视觉 Must avoid、§12 交接摘要。其余章节保持 v1 原文不动。
...
- **对标复用策略 rev2（owner 授权 / owner-authorized）**：以 A（MistfallDB）工具链、B（mistfallhunters.wiki）内容结构、C（mistfallhunters.com）信任标签为直接对标基准；owner 允许汲取/复用竞品与官方的图片、数据、文案方向和主题，必要时可直接用——全部复用逐条进复用台账，IP/版权/ToS/商标风险由 04 合规评估缓解与 takedown 预案（见 §4 / §11）。
...
### 2.2 逐竞品拆解 / Per-competitor analysis（借鉴点 / 可复用点 / 可超越点 / 风险点）

**rev2 对标复用政策（owner 已批准 / owner-approved reuse policy）**：对标基准从 v1「internal benchmark only / 仅内部对标」升级为「可复用对标」。执行口径：
1. 竞品的优点——结构、信息架构、工具功能、交互范式——允许直接借鉴并落到本站实现；
2. 竞品与官方的**图片、数据、文案方向、主题**允许汲取/复用，必要时可直接用；
3. 每一份复用的图片/数据/文案/主题必须进入**复用台账**（`inputs/04-compliance/reuse-ledger.md`，字段：素材 ID / 来源 URL / 复用位置 / 复用方式[原样|改编] / 权属状态 / 登记日期；04 合规阶段建立并审计，先登记后使用）；
4. 品牌类使用（竞品站名/logo、官方商标/logo 本体）不在"直接可用"范围：官方 logo 本体禁用（改原创类官方风格 logo，见 §4 N2），竞品品牌名称/logo 的公开使用由 04 合规在 takedown 预案口径内评估；
5. 相关 IP/版权/游戏 ToS/商标相似风险全部进 §11 风险登记，由 04 合规评估缓解措施与 takedown 预案。
...
- 可直接复用 reuse（rev2 放行，逐条进台账）：物品图片素材、结构化数据（武器/护甲/掉落获取途径）、数据文案方向与主题；复用范围与限度由 04 合规评估（见 §11 R10）。
- 可超越 surpass：它没有 Squad Builder / Class Quiz / Matchup Matrix / Codes 追踪 / 补丁时间线 / 新手引导 / Duo 内容——这 7 项全是我们的 P0/P1。
- 风险点 risk：599 武器 / 1584 护甲的数据护城河短期追不上 → 主打决策工具差异化；数据页 v1 覆盖高频子集，并可通过台账内复用加速补齐（rev2 变化：不再把"不做全量复刻"写死为边界，改为按台账+合规口径逐步扩量）。
...
- 可直接复用 reuse（rev2 放行，逐条进台账）：内容主题、选题角度、攻略图片、数据与文案方向。
...
- 可直接复用 reuse（rev2 放行，逐条进台账）：信任表达方式、相关图片、数据与文案方向。
...
### 定位策略（rev2 修订）/ Positioning strategy
**对标复用型决策引擎**：以 MistfallDB 的工具链、mistfallhunters.wiki 的内容结构、mistfallhunters.com 的信任标签为直接对标基准，owner 授权汲取/复用竞品与官方的优点——图片、数据、文案方向和主题，必要时可直接用（逐条进复用台账）；在此之上用独家互动决策工具（Squad Builder / Class Quiz / Matchup Matrix / Settings Recommender）与移动端体验实现超越。
决策引擎定位本身不变：竞品是查数据/读攻略/验事实，我们是做决策；粘性来自工具输出可分享、可迭代。
...
### NOT-DO 明确不做（产品边界红线，rev2 重写为可执行口径）

> rev2 依据：owner 2026-08-08 部分修改确认（inputs/02-owner-confirmations/owner-decision-02A-rev-20260808.md）。下表 #2、#3、#5、#6、#12 为 owner 明确决策放行的方向；相应 IP/版权/游戏 ToS/商标相似风险已按 owner 决策写入 §11 风险登记（R8-R11），由 04 合规阶段评估缓解措施与 takedown 预案。

| # | 边界 Boundary（rev2 口径） | 原因与处置 |
|---|---|---|
| N1 | ❌ 不暗示官方身份；全站页脚标注 "Unofficial fan resource. Not affiliated with Bellring Games or Skystone Games." | 启动卡硬约束 + DMCA/UDRP 风险；此条不因 rev2 放宽 |
| N2 | ❌ 不使用官方 logo 本体（含官方 logo 原图/官方 logo 微调版）；✅ 做一个**类似官方风格的原创 logo**（原创绘制，视觉气质接近官方但图形可区分） | owner 明确决策；logo 相似度与商标混淆风险见 §11 R10；06 设计阶段出稿 + 04 合规复核，保留原创过程证据 |
| N3 | ❌ 不暗示官方背书/授权/合作 | 与 N1 同源；About/Contact 文案由 04 合规定稿 |
| N4 | ❌ 不冒用竞品品牌：不把竞品站名/logo 用作本站品牌、域名、页面主视觉 | 品牌混淆风险；竞品元素的具体复用边界由 04 合规按台账口径评估 |
| N5 | ✅ 允许扒/采集游戏数据（官方与竞品公开数据均可爬取、整理、入库） | owner 明确决策"肯定要有游戏数据"；每条数据进来源台账（来源 URL / 采集日期 / Last Verified）；游戏 ToS / 法律风险见 §11 R9，缓解与 takedown 预案由 04 评估 |
| N6 | ✅ 允许汲取/复用竞品与官方的图片、数据、文案方向和主题，必要时可直接用 | owner 明确决策；逐条进复用台账（先登记后使用）；版权/ToS 风险见 §11 R8/R9，缓解与 takedown 预案由 04 评估 |
| N7 | ❌ 不做视频内容 | YouTube 生态已饱和 |
| N8 | ❌ 不做论坛/社区 | Reddit/Discord 已存在 |
| N9 | ❌ 不做新闻/更新速度竞赛 | wiki 类站已覆盖；我们做补丁后的"Meta 解读"而非快讯 |
| N10 | ❌ 不做商店/电商/代练 | 非产品方向 |
| N11 | ❌ 不做 P2W 相关内容 | 游戏非 P2W，维护社区信任 |
| N12 | ❌ 首版不做登录/注册/付费墙 | 全部工具匿名可用；变现由 03 定价阶段单独评估并走 owner 闸门 |

**v1 → rev2 变化说明 / Change log**：
- v1 #3「不自动爬取游戏数据」→ 删除，改为 N5（owner 确认要扒游戏数据）。
- v1 #4「不复制竞品文案/图表/设计/配色编码」→ 删除，改为 N6（owner 允许复用，必要时直接用，台账管理）。
- v1 #2「不使用官方 logo / 官方素材做站点品牌」→ 收窄为 N2（仅禁官方 logo 本体及其微调版，原创类官方风格 logo 放行）；官方素材复用并入 N6。
- v1 #10「v1 不做全量物品数据库」→ 移出 NOT-DO：数据量策略改为"v1 高频子集起步，按台账+合规口径逐步扩量"（见 §2.2 A 风险点）；其余 v1 条目（#1/#5-#9/#11/#12）原样保留为 N1/N3/N7-N11/N12。

### 复用台账规范（rev2 新增）/ Reuse ledger requirement
- 路径：`inputs/04-compliance/reuse-ledger.md`（04 合规阶段建立并审计；文案/设计/前端先登记后使用）。
- 字段：素材 ID / 类型（图片|数据|文案|主题）/ 来源（官方|竞品+URL）/ 复用位置（route）/ 复用方式（原样|改编）/ 权属状态 / 登记日期。
- 用途：04 合规按台账逐项评估版权与 ToS 暴露面，并据此写 takedown 预案（收到通知后按台账定位、下架/替换的 SOP）。
...
### 5.6 数据合同基线 / Data contract baseline（rev2 修订；细节在 PRD V1）
- 游戏数据来源（rev2）：**允许爬取/采集官方与竞品的公开数据**（owner 决策；v1 的"不爬取、仅人工整理"口径作废）；每条关键数据带来源链接、采集日期与 Last Verified 日期；爬取频率与礼貌抓取策略（rate limit / robots 检查）由 08 后端在实现时落实，法律/ToS 暴露面由 04 合规评估（§11 R9）。
- 图片与素材（rev2）：**允许复用官方与竞品素材，必要时可直接用**（owner 决策）；每份复用素材进复用台账（见 §4 复用台账规范）；**logo 除外**——站点 logo 必须原创（类官方风格，见 §4 N2）。素材权属与下架预案由 04 合规 + 06 设计复核。
...
- **设计语言**：卡片式布局（游戏 wiki 标准范式）；**游戏背景蒙版动效 Hero**（owner 要求：沉浸感，动效循环不干扰 CTA；rev2：背景素材允许使用官方图片/原画，进复用台账）；工具首屏可操作；mobile-first（320~1920 全断点，重点 360/390/430）；Mega Menu 五组导航。
- **Logo（rev2）**：不用官方 logo 本体；做类似官方风格的**原创 logo**（06 设计出稿，保留原创过程证据，04 合规评估相似度）。
- **Must avoid 禁止（rev2 收窄）**：不冒用竞品站名/logo 做本站品牌（N4）；不使用官方 logo 本体或其微调版（N2）。其余素材（官方/竞品图片、数据、文案方向、主题）按 §4 N6 复用台账口径放行。
...
## 11. 风险评估 / Risk Assessment（rev2：新增 IP/版权/ToS/商标风险登记）

**P0（必须处理）**
| # | Risk 风险 | Mitigation 缓解 |
|---|---|---|
| R1 | 品牌 DMCA / UDRP（域名含游戏名） | 全站 Unofficial 声明；不用官方 logo 本体；About 明示非官方；04 合规复核 |
...
| R8 | **IP/版权：复用官方与竞品的图片/数据/文案/主题可能触发版权主张、DMCA 通知或平台下架**（rev2 新增；owner 已明确决策接受该复用方向） | 复用台账逐条登记（先登记后使用）；来源标注；**04 合规阶段产出缓解措施 + takedown 预案**（收到通知后按台账定位并下架/替换的 SOP） |
| R9 | **游戏 ToS / EULA：扒取游戏与竞品公开数据、复用官方素材可能违反 Bellring/Skystone 的 ToS、EULA 或粉丝内容政策**（rev2 新增；owner 已明确决策接受） | 礼貌抓取（rate limit / robots 检查，08 后端落实）；数据/素材台账；**04 合规阶段逐条评估 ToS 暴露面并给出缓解措施 + takedown 预案** |
| R10 | **商标/Trade dress 相似：原创 logo"类似官方风格"与站点名称含游戏名，可能被主张商标混淆或视觉近似**（rev2 新增；owner 已明确决策采用类官方风格原创 logo） | logo 原创绘制（禁用官方 logo 本体/微调版）；保留设计过程证据；图形与官方 logo 保持可区分度；**04 合规评估相似度阈值与改稿预案** |
| R11 | 竞品反应：直接复用竞品素材/结构可能招致竞品站方投诉或反制（rev2 新增） | 复用台账区分"官方来源/竞品来源"并分级（直接复用项优先用官方来源）；takedown 预案覆盖竞品投诉路径 |

**P1（需要关注）**
| # | Risk | Mitigation |
|---|---|---|
| R4 | MistfallDB 数据护城河 | 工具差异化竞争；数据小而准逐步补（rev2 起可经台账复用加速） |
...
**风险登记状态 / Risk register status**：R8-R11 为 owner 明确决策接受的方向性风险；缓解措施与 takedown 预案不在本 brief 范围内定稿，属 **04 合规阶段必交付项**（验收口径见 §12）。
...
| 04 合规 | §4 NOT-DO（N1-N12）+ 复用台账规范 + §5.6 数据合同 + §11 风险登记（R1/R2/R8-R11）+ inputs/02-owner-confirmations/owner-decision-02A-rev-20260808.md | **必交付**：R8-R11 缓解措施 + takedown 预案（按复用台账定位下架/替换 SOP）+ 复用台账建立与审计 + Privacy/Terms/Cookie/Disclaimer + logo 相似度评估 |
| 05S SEO | §7 Route Contract + Title/Meta 格式（PRD V1 逐页回填） | Title/Meta/Schema/Sitemap 执行基线 |
| 05 文案 | §3 ICP + §5/§6/§7 + §2.2 复用政策 | 58 页文案（英文）；复用素材先登记后使用 |
| 06 设计 | §6/§9 + §5 工具交互 + §4 N2 | 视觉真源 + 移动端首屏；**logo 必须原创（类官方风格），保留原创过程证据** |
| 07/08 前后端 | §8 技术栈 + §7 Route Contract + §5.6 数据合同 | Next.js + Workers (OpenNext) + D1/R2；爬虫礼貌抓取策略（08 后端） |

**不能假设 / Cannot assume**：域名已注册；游戏数据已结构化（需采集整理）；工具已实现；游戏不会更新；复用素材的权属已评估（必须经 04 合规 + 台账）。
**仍锁定 / Still locked**：PRD V1、定价、合规执行、SEO 基线、文案、设计、实现、部署、一切公开动作——全部等 02B（rev2 确认）及后续 owner 闸门。

---

## Owner 确认清单 / Owner Confirmation Checklist（02B，rev2 状态）

| # | 确认项 | 状态 | 说明 |
|---|---|---|---|
| 1 | 站点类型 Site type | ✅ 已确认（2026-08-08） | 决策引擎混合站：工具 60% + 内容 30% + 信任体系 10%，非官方粉丝站 |
| 2 | 产品定位 Positioning | 🔁 rev2 修订，待最终确认 | 对标复用型决策引擎：允许汲取/复用竞品与官方的图片、数据、文案方向和主题，必要时可直接用；肯定要有游戏数据（扒取放行）；见 §4 定位策略 + §2.2 复用政策 |
| 3 | 独家差异化工具 Differentiators | ✅ 已确认（2026-08-08） | Squad Builder / Class Quiz / Matchup Matrix / Settings Recommender / Duo 内容 / Patch Tracker |
| 4 | 首版工具分批 Tool phasing | ✅ 已确认（2026-08-08） | V1 首发 6 个 + V1.5 上线后 2-4 周 4 个；58 路由与 48 内容页全部 V1 存在 |
| 5 | 首版页面规模 Page count | ✅ 已确认（2026-08-08） | 58 个可索引路由，每页独立意图/H1/FAQ/内链，不做薄页 |
| 6 | NOT-DO 红线 | 🔁 rev2 修订，待最终确认 | §4 N1-N12 可执行口径：扒游戏数据放行（N5）；复用竞品/官方图片、数据、文案方向、主题放行（N6，台账制）；logo 不用官方本体，做类官方风格原创 logo（N2）；IP/版权/ToS/商标风险进 §11 风险登记，交 04 合规评估缓解与 takedown 预案 |
| 7 | 域名方向 Domain | ✅ 已确认（2026-08-08） | 首选 mistfallhunter.gg（未注册，等 owner 注册授权，本阶段不注册）；.co 可选 301 |
| 8 | 技术栈与视觉 Tech & visual | ✅ 已确认（2026-08-08） | Next.js + Cloudflare Workers (OpenNext, 不用 Pages) + D1 + R2；深色+金、卡片式、蒙版动效 Hero、mobile-first |

**本次只需确认 #2 与 #6 两项（rev2 修订口径）。**

**回复格式 / Reply format**
- 全部确认：`确认 rev2，进入 PRD V1`
- 部分修改：`修改 [编号] 为 [新方向]，其余确认`
- 需要讨论：`[编号] 不确定，展开讲讲`

---

**Status**: [NEEDS_REVIEW] — rev2 已就绪，等待 owner 对 #2/#6 修订口径做最终确认（02B）；未确认前不启动 PRD V1 及任何下游。
